...
export function AssignStudentsToGroupDialog({
  open,
  onOpenChange,
  schoolId,
  groupId,
  ...
  gradeId,
  onSuccess,
}: AssignStudentsToGroupDialogProps) {
  ...
  const fetchStudents = async () => {
    try {
      ...
      // Filtrar estudiantes del mismo grado que no están en este grupo
      const availableStudents = data.school.users.filter(
        (u: any) => 
          u.roles.includes("STUDENT") &&
          u.studentGroups.some((sg: any) => sg.group.gradeId === gradeId) &&
          !u.studentGroups.some((sg: any) => sg.group.id === groupId)
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // TODO: Implementar API para asignar estudiantes a grupo
      ...
      toast.success(`${selectedStudents.size} estudiante${selectedStudents.size !== 1 ? 's' : ''} asignado${selectedStudents.size !== 1 ? 's' : ''} al grupo`);
      onSuccess();
      resetForm();
    } catch (error: any) {
      console.error("Error:", error);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Asignar Estudiantes a {groupName}</DialogTitle>
          <DialogDescription>
            Selecciona los estudiantes del grado que deseas agregar a este grupo
          </DialogDescription>
        </DialogHeader>
          ...
          {fetchingStudents ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm mt-2">
                Todos los estudiantes del grado ya están en este grupo o no hay estudiantes en el grado
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              ...
              <Label>
                Estudiantes del grado disponibles ({filteredStudents.length})
              ...
              <div className="border rounded-md max-h-96 overflow-y-auto">
                {displayedStudents.length === 0 ? (
                  ...
                  >
                    ...
                    {student.studentGroups.length > 1 && (
                      <Badge variant="secondary" className="text-xs">
                        En {student.studentGroups.length - 1} otro{student.studentGroups.length - 1 !== 1 ? 's' : ''} grupo{student.studentGroups.length - 1 !== 1 ? 's' : ''}
                      </Badge>
                    )}
                    </div>
                  ))}
